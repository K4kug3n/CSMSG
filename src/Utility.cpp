...
std::array<unsigned char, 32> to_bytes(const mp::uint256_t& x) {
	std::array<unsigned char, 32> bytes;
	for (size_t i = 0; i < bytes.size(); ++i) {
		bytes[i] = static_cast<unsigned char>((x >> (i * 8)) & 0xFF);
	}
...
mp::uint512_t to_integer(const std::array<unsigned char, 64>& arr) {
	mp::uint512_t x = 0;
...
mp::uint256_t to_integer(const std::array<unsigned char, 32>& arr) {
	mp::uint256_t x = 0;
	for (size_t i = 0; i < arr.size(); ++i) {
